...
public class LoginClienteServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            App.authService.isValidUserCliente(dni, pwd);// si falla se genera una excepcion InvalidUser
            // Crear sesion si es valid user
            HttpSession ses = req.getSession();
            ses.setAttribute("is_valid_user", true);
            ses.setAttribute("username", dni);
            resp.sendRedirect("dashboardCliente"); // Se reenvia al servlet /dashboardCliente
