...
@WebServlet(name = "loginEmprendedor", urlPatterns = "/loginEmprendedor")
public class LoginEmprendedorServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            App.authService.isValidUserEmprendedor(dni, pwd);// si falla se genera una excepcion InvalidUser
            // Crear sesion si es valid user
            HttpSession ses = req.getSession();
            ses.setAttribute("is_valid_user", true);
            ses.setAttribute("username", dni);
            resp.sendRedirect("dashboardEmprendedor"); // Se reenvia al servlet /dashboardEmprendedor
